...
FROM workers
GROUP BY state 
HAVING COUNT(state)>1;  -- having , Group Bay ardından filtrelemek için kullanılıyor 
...
from workers ;  -- DISTINCT clause , çağırılan terimlerden  tekrarlı olanların sadece birincisini alır 
...
--salary değeri 3000'den yüksek olan state değerlerini ve 2000'den küçük olan name değerlerini
--tekrarlı olarak bulun.
SELECT state AS "Name and State", salary 
FROM workers
WHERE salary>3000
UNION ALL    ---> UNION ile aynı işi yapar. Ancak tekrarli değerleride verir 
SELECT name, salary 
FROM workers
WHERE salary<2000;
...
--EXCEPT Operator : (Hariç demek) Bir sorgu sonucundan başka bir sorgu sonucunu çıkarmak için kullanılır. 
...
/*
JOINS :1) INNER JOIN: Ortak (Common) datayı verir.İKİ farklı tablounun ortak elemanlarını  verir , kesişimidir
