...
select isim,id,yas, email from personel 
where id=5;
...
select * from personel where isim ilike 'A%';  -- % işareti ne ile devam ediyorsa etsin demek 
...
select * from personel where id in (3,5,7); -- kısa yazılışı 
select * from personel where id= 3 or id=5 or id=7 or id=9; -- uzun yazılışı 
...
select * from personel where  yas not between 30 and 40; -- 1 .çözüm yolu
select * from personel where yas<30 or yas>40;   -- 2.çözüm yolu
...
select * from personel order by yas; -- dogal sıralama küçükten büyüğe sıralar asc de yazılabilir ancak default dur 
